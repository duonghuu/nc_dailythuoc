...
<form autocomplete="off" name="supplier" id="validate" class="form" action="index.php?com=product&act=save&p=<?=$_REQUEST['p']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
 <div class="widget">
  ...
  if(in_array('phaply',$config['type'])) { 
        $s_day = ($item['phaply']>0)?$item['phaply']:time();
        $dt_format = date('d-m-Y',$s_day);
        
    ?>
    <div class="formRow">
      <label><?= (!empty($config["title"]["phaply"]))?$config["title"]["phaply"]:'Pháp lý' ?>:</label>
      <div class="formRight">
        <input type="text" id="ngayketthuc" name="phaply" value="<?=$dt_format?>" readonly="" title="Nhập" class="tipS" />
      </div>
      <div class="clear"></div>
    </div>
  <?php } ?>
