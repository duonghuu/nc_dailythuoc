			...
			if($d->insert($isrt_data))
			{
				...
				#Nếu insert bảng chitietdonhang thành công thì bắt đầu gửi mail
					...
					// $mail->CharSet = "utf-8";		
						...
						if(is_array($_SESSION['cart']))
						{
							...
							for($i=0;$i<$max;$i++){
								...
								$body.='<td width="15%" align="center"><img src="https://'.$config_url.'/upload/sanpham/'.$pphoto.'" style="max-height:50px; margin:5px 0;" /></td>';
				   ...
				   #------------Chi tiết đơn hàng---------------------
						...
						$from=$mail_host;
						$from_name=$subject;
						// The following From address failed: lienhe.nhathuocgiangson@gmail.com
						$more=array("email"=>$company['email']);
						if(sendEmail($sendType,$to, $from, $from_name, $subject, $body,$more,$file=''))
						{
							transfer("Bạn đã đặt hàng thành công.<br> Chúng tôi sẽ liên hệ với bạn sớm nhất.<br>Mã đơn hàng là: ".$madonhang, "https://".$config_url);
						}
						else{
							transfer("Bạn đã đặt hàng thành công.<br> Chúng tôi sẽ liên hệ với bạn sớm nhất.<br>Mã đơn hàng là: ".$madonhang, "https://".$config_url);
						}
					...
					transfer("Đơn hàng của bạn chưa có sản phẩm<br>Vui lòng chọn sản phẩm để đặt hàng<br>Cảm ơn", "https://".$config_url);
				}
			}
			else
			{
				transfer("Xin lỗi quý khách.<br>Hệ thống bị lỗi, xin quý khách thử lại.", "https://".$config_url);
			}
